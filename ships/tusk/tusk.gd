...
@export var min_shape := Vector2(min_size[2].x, (min_size[1].y * 2) - 2) #Vector2(25, 6)
@export var max_size: PackedVector2Array = [Vector2(0, -6), Vector2(0, 6), Vector2(75, 0)]
@export var max_shape := Vector2(max_size[2].x, (max_size[1].y * 2) - 2) #Vector2(50, 8)
...
@export var damage_knockback_ratio := 8.0
...
func _ready() -> void:
	super()
	lance.polygon.polygon = min_size
	lance.collider.shape.size = min_shape
...
func _physics_process(delta: float) -> void:
	...
	if not lance.reflect_mode:
		...
		var speed_ratio := inverse_lerp(0, SPEED_LIMIT, linear_velocity.length())
		for i in range(min_size.size()):
			new_size[i] = min_size[i].lerp(max_size[i], speed_ratio)
		...
		lance.collider.position.x = lance.collider.shape.size.x / 2
		
		lance.attack.attack_damage = lerp(min_damage, max_damage, speed_ratio)
		lance.attack.knockback_force = lerp(min_damage, max_damage, speed_ratio) * damage_knockback_ratio
...
func secondary() -> void:
	...
	#lance.attack.attack_damage = lance_damage
	#lance.attack.knockback_force = lance_damage * damage_knockback_ratio
	var stash_polygon := lance.polygon.polygon
	var stash_size := (lance.collider.shape as RectangleShape2D).size
	...
	thrust_tween.tween_property(lance.collider, "position:x", parry_shape.x/2, tween_time)
	thrust_tween.tween_callback(func():
		lance.enabled = false
		lance.reflect_mode = true)
	thrust_tween.set_ease(Tween.EASE_IN).chain().tween_interval(parry_time)
	thrust_tween.chain().tween_property(lance.polygon, "polygon", stash_polygon, tween_time)
	thrust_tween.tween_property(lance.collider.shape, "size", stash_size, tween_time)
	thrust_tween.tween_property(lance.collider, "position:x", stash_size.x/2, tween_time)
	thrust_tween.tween_callback(func():
		lance.enabled = true
		lance.reflect_mode = false)
